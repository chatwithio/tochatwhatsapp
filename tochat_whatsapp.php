...
class Tochat_Whatsapp extends Module
{
    ...
    public function install()
    {
        ...
        CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'tochat_whatsapp_message` (
              ...
              CONSTRAINT `TOCHAT_WHATSAPP_MESSAGE_ORDER_ID_PS_ORDERS_ID_ORDER` FOREIGN KEY (`order_id`) REFERENCES `ps_orders` (`id_order`) ON DELETE SET NULL
    ...
    public function hookActionObjectOrderAddAfter(array $params)
    {
        ...
        if(!$automationStatus) return true;
    ...
    protected function _postValidation()
    {
        if (Tools::isSubmit('tochatwhatsapp_automation') 
            && Tools::getValue('TOCHATWHATSAPP_AUTOMATION_STATUS') == 1 ) {
    ...
    public function getApiObject($apikey = null, $endpoint = null){
        if(!$apikey) $apikey = Configuration::get('TOCHATWHATSAPP_AUTOMATION_APIKEY');
        if(!$endpoint) $endpoint = Configuration::get('TOCHATWHATSAPP_AUTOMATION_ENDPOINT');
    ...
    public function hookDisplayAdminOrderSideBottom($params){
        ...
        return $this->display(__FILE__,"views/templates/customer_telephone_field.tpl");
    }
    ...
    public function displayCronUrl(){
        ...
        $token = substr(Tools::encrypt('tochat_whatsapp/cron'), 0, 10);
        $this->smarty->assign([
            'cron_url'=>$domain . __PS_BASE_URI__ . 'modules/tochat_whatsapp/cron.php?&token='.$token
        ]);
        return $this->display(__FILE__,'views/templates/cron-url.tpl');
    }
    ...
    public function automateMessage(){
        ...
        if(!count($messesges)) return;

        global $currency;
        $currencySym = $currency->sign;
    ...
    public function abandonedCart(){
        ...
        $interval = intval((float)Configuration::get('TOCHATWHATSAPP_ABANDONED_INTERVAL', 10) * 60);
        ...
        foreach ($carts as $cart){
            ...
            }else if($cart['id_address_invoice']){
